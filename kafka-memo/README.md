...
- kafka に送信されたイベントをリアルタイムに処理するための API とランタイムを提供する Java クライアントライブラリ
- Kafka Streams はストリーム処理を実装するための抽象化レイヤーを提供する
- ストリーム処理では、加工、フィルタ、集計、複数の Topic のイベントの結合などの操作ができる
  - これらの操作を定義した Streams DSL が用意されているため、Producer や Consumer と比較して簡潔に処理を実装できる
  - また Streams DSL で定義されていない操作や低レイヤー処理を実装する場合は Processor API も用意されている
- Kafka Streams のユースケース
  - 個人情報のイベントをマスキングして、データレイクやデータウェアハウスに流し込む
  - 店舗ごとの一定時間ウィンドウ内の売り上げの合計をリアルタイムに計算して BI ツールで表示する
  - DB のユーザー情報のテーブルと商品取引のストリームデータを結合して、取引情報にユーザー属性を付与する
- Kafka Streams の構成

  - プロセッサーと呼ばれる処理単位から構成される
  - プロセッサーは Kafka から読み取ったイベントに対して処理を実行する
  - プロセッサーの処理内容は以下
    - ソースプロセッサー
      - Kafka Topic からイベントを読み取ります
    - シンクプロセッサー
      - ストリーム処理の終端処理を実行し、イベントを Kafka Topic に送信したりステートストアに保存する
    - ストリームプロセッサー
      - イベントの加工、フィルタ、集約、結合などのストリーム処理を実行
  - これらのプロセッサーの処理結果はまた別のプロセッサーが受け取り、処理が連鎖する構造をとる

    ![kafka-streams1](./setup/connect-cluster/kafka-streams1.png)

- Kafka Streams の使用の要件

  - チケット販売システムを利用したサービスはユーザー数の増加と共にチケット取引量が増えている。一方他のユーザーのチケット販売を妨害、あるいは利用規則に沿わない不正な取引の増加に悩まされている。チケット販売で不正な手段で大量に購入転売する行為を防止するために、不正な取引を検知する目的で以下の機能要件を実現する

    - 転売が多発している「ライブ」カテゴリのチケット取引を対象に不正を検知できる
    - 短時間に複数の取引を行ったユーザーを不正疑いとして抽出できる

      ![kafka-streams2](./setup/connect-cluster/kafka-streams2.png)

      ![kafka-streams3](./setup/connect-cluster/kafka-streams3.png)

    ## Strimzi

    - Kubernetes 上で Kafka クラスタを管理するオープンソースの Kubernetes Operator
      - カスタムリソースで定義する
      - マニフェストで宣言的に Kafka クラスタのコンポーネントを管理できる
      - Topic の作成・更新、認証ユーザーの作成・削除もカスタムリソースで操作可能
